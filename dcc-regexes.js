...
if (!this.PriceRegexes) {
    const PriceRegexes = (function() {
        ...
        const makePriceRegexes = function(aRegex1, aRegex2) {
            const begin = "(?:^|\\s|\\()";
            const value = "((?:\\d{1,3}(?:(?:,|\\.|\\p{Zs}|')\\d{3})+|(?:\\d+))(?:(?:\\.|,)\\d{1,9})?)";
            const space = "(?:\\s?)";
            ...
            const makeRegex1 = function(reg) {
                console.log(begin + reg + space + value);
            ...
            try {
                ...
                aRegex2.CHF = RegExp("((\\d{1,3}(('|,|\\.|\\s)\\d{3})+|(\\d+))((\\.|,)\\d{1,9})?\\s?)(CHF|Fr\\.|Franken|Rappen|centimes?|centesimo|centesimi|rap)" + end, "g");
