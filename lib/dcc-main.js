...
//                request3.get();
        ...
        return {
            ...
            registerToTabsEvents: () => {
                const setTabs = (aTab) => {
                    ...
                    else if (aTab.url === "resource://jid1-ep6btq0bhcevya-at-jetpack/direct-currency-converter/data/prices.html") {
                        testPageWorker = aTab.attach({contentScriptFile: [aUrlProvider.getUrl("dcc-regexes.js"), aUrlProvider.getUrl("dcc-content.js")]});
                        const finishedTabProcessingHandler = (aHasConvertedElements) => {
                            try {
                                if (testPageWorker.tab.customTabObject == null) {
                                    testPageWorker.tab.customTabObject = new CustomTabObject();
                                }
                                testPageWorker.tab.customTabObject.isEnabled = anInformationHolder.isConversionEnabled();
    ...
    const BarsInterface = function(aUrlProvider, anInformationHolder) {
        ...
        settingsPanel.port.on("showSettingsTab", function () {
            eventAggregator.publish("right-click");
            settingsPanel.hide();
        });
        settingsPanel.port.on("showTestTab", function () {
    ...
    const Controller = function(anEventAggregator) {
        ...
        anEventAggregator.subscribe("right-click", () => {
