...
const FirefoxContentInterface = function(anInformationHolder, PageMod, ContentScriptParams, tabs, eventAggregator) {
    ...
    const watchForPages = () => {
        ...
        const setTab = (aTab) => {
            if (aTab.url
                ...
                settingsWorker.port.on("resetQuotes", () => {
                ...
                && aTab.url.includes("dcc")
                && aTab.url.includes("joint-dot-ax")
                && aTab.url.includes("quotes.html")) {
                // PageMod does not seem to work on resource URLs
                quotesPageWorker = aTab.attach({contentScriptFile: ["./common/dcc-quotes.js", "./dcc-firefox-quotes-adapter.js"]});
                quotesPageWorker.port.emit("updateQuotes", new ContentScriptParams(quotesPageWorker.tab, anInformationHolder));
            }
    ...
    const showQuotesTab = () => {
        const isOpen = quotesPageWorker && quotesPageWorker.tab ;
        if (!isOpen) {
            tabs.open({url: "./common/quotes.html"});
        }
        else {
            quotesPageWorker.tab.activate();
        }
    };
