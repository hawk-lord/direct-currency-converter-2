...
const YahooQuotesServiceProvider = function(anEventAggregator) {
    ...
    const quoteQueriesFromTo = [];
    const quoteQueriesToFrom = [];
    var quotesFromTo = [];
    var quotesToFrom = [];
    ...
    const quotesHandlerFromTo = function(aResponse) {
        try {
            ...
            if (aResponse.status == "200") {
                ...
                // console.log("quotesFromTo.length " + quotesFromTo.length);
    ...
    const quotesHandlerToFrom = function(aResponse) {
        try {
            ...
            if (aResponse.status == "200") {
                ...
                // console.log("quotesToFrom.length " + quotesToFrom.length);
                if (quotesFromTo.length > 0 && quotesToFrom.length > 0) {
                    makeOneResponse();
                }
            }
